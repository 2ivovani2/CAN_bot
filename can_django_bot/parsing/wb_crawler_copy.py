...
class WildberriesCommentsSpider1(BaseSpider):
    name = "wb_comments"

    def __init__(self, good_url, *args, **kwargs):
        super(WildberriesCommentsSpider1, self).__init__(*args, **kwargs)
...
def f1(runner: CrawlerRunner, link:str) -> None:
    """
    ...
    deferred = runner.crawl(WildberriesCommentsSpider1, good_url=link)
    deferred.addBoth(lambda _: reactor.stop())
    reactor.run(installSignalHandlers=False)
    
    return None

def parse_product_1(link:str, save_filename:str='data_') -> Tuple[str, str, pd.DataFrame]:
    '''
    ...
    p = Process(target=f1, args=(runner, link))
    p.start()
    p.join()
